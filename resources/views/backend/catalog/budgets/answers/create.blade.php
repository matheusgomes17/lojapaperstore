...
<section class="invoice">
	...
	<div class="row budget_method_payment">
		<!-- accepted payments column -->
		<div class="col-xs-12">
		  	<p class="lead">{{ trans('labels.backend.catalog.budgets.title') }}:</p>
		  	{!! Form::model($budget, ['method' => 'POST', 'route' => ['admin.catalog.budget.answer.store', $budget->id]]) !!}
		  	{{ Form::hidden('budget_id', $budget->id) }}
	  		{{ Form::textarea('answer', null, ['id' => 'budget', 'class' => 'textarea']) }}
		</div>
		<!-- /.col -->
	</div>
	<!-- /.row -->
	...
	<div class="row no-print">
		<div class="col-xs-12">
	  		<a href="{{ route('admin.catalog.budget.index') }}" class="btn btn-danger"><i class="fa fa-arrow-left"></i> {{ trans('buttons.general.cancel') }}</a>
